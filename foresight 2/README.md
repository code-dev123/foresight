...
Stage 1 MVP — Data Collector. Analyzers implemented:

- ✅ **Istio AuthorizationPolicy** — detects traffic sources blocked by proposed ALLOW rules
- ✅ **Istio VirtualService** — detects weight shifts, destination changes, route additions/removals, and policy changes
...
./bin/foresight analyze -f test/fixtures/istio-virtualservice-canary.yaml --dry-parse

# Real analysis against your cluster (requires Istio + Prometheus)
./bin/foresight analyze -f my-change.yaml

# VirtualService canary promotion — weights shifted to 100% v2
./bin/foresight analyze -f test/fixtures/istio-virtualservice-canary.yaml
